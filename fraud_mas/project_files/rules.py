# Function execute_fraud_rules reads the data elements from the stream and scores the data on three counts
# 	a. it calculates a fraud score by scoring the transaction against "analyst" defined pre-set rules that attempts to capture the "suspect nature" of a transaction
#	b. it also computes a merit score by scoring the transaction against "analyst" defined pre-set rules that attempts to capture the "goodness" of a transaction
#	c. finally both a. & b. are used to compute an outsort score. This in other words is a "rules-based-alert-score" 
